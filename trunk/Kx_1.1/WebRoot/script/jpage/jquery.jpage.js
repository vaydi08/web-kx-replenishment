   ...
   */
	jQuery.fn.jpage = function(config){
		...
		function init(t,config){
			...
			var showPerPage = config.showPerPage != null ? config.showPerPage : true;
			var configPage = config.perPage > 0 ? config.perPage : 40;
			...
			function loadData(){
				...
				var view = "";
				//alert(startRecord + ' '+endRecord + ' ' + dataStore.length);
				for(var i=startRecord-1;i<=(endRecord>dataStore.length?dataStore.length:endRecord)-1;i++){
