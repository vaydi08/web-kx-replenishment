(function($) {
	$.fn.info_CategoryManager = function() {
			...
			SF.submit(ctrl.Form,{
				...
				onSubmit:function(){
					if(ctrl.Input.Clevel.combobox('getValue') == 4) {
						if($('#INFO_C_capTable').find('input[name=picType]:checked').val() == "webcap" && $('#INFO_C_capTable').find(ctrl.Pic.picData).val() == "") {
							SOL.showWarning('请首先使用摄像头程序照相 或上传一个图片文件');
							return false;
						}
						if($('#INFO_C_capTable').find('input[name=picType]:checked').val() == "file" && $('#INFO_C_capTable').find(ctrl.Pic.picFile).val() == "") {
							SOL.showWarning('请首先使用摄像头程序照相 或上传一个图片文件');
							return false;
						}
					}
		...
		function init() {
			...
			ctrl.QueryPanel.Type1.combobox({
				...
				onSelect:function(row) {
					if(row.value > 0)
						ctrl.QueryPanel.Type2.combobox('reload',"../info/info-category!typeQueryCombo.action?input.clevel=2&input.parent=" + row.value);
				}
			});
			ctrl.QueryPanel.Type2.combobox({
				...
				onSelect:function(row) {
					if(row.value > 0)
						ctrl.QueryPanel.Type3.combobox('reload',"../info/info-category!typeQueryCombo.action?input.clevel=3&input.parent=" + row.value);
				}
			});
			...
			// 对话框内控件 父类别
			ctrl.Input.Parent.combobox({url:'../info/info-category!newTypeCombo.action?input.clevel=0'});
			// 对话框内控件 类别
			ctrl.Input.Clevel.combobox({
				onSelect : function() {
					var clevel = ctrl.Input.Clevel.combobox('getValue');
					if(clevel > 1) {
						ctrl.Input.Parent.combobox('reload','../info/info-category!newTypeCombo.action?input.clevel=' + (clevel-1));
					} else {
						ctrl.Input.Parent.combobox('reload','../info/info-category!newTypeCombo.action?input.clevel=0');
		...
		ctrl.Toolbar.Del.click(function() {
			SF.confirm(ctrl.ListTable,'../info/info-category!delete.action');
		});
		ctrl.Toolbar.Edit.click(function() {
			...
			if(row) {
				$.messager.prompt('修改','只允许修改类别名称,请输入新的名称',function(val){
					if(val) {
						$.post('../info/info-category!edit.action',{"input.id":row.id,'input.cname':val},function(data){
							var result = eval('(' + data + ')');
							if(result.success)
								ctrl.ListTable.datagrid('reload');
							else
		...
		ctrl.Pic.Startcap.click(function() {
			SF.openCap(function(result){
				$('#INFO_C_capTable').find(ctrl.Pic.picData).val(result);
			});
		});
	}
})(jQuery);