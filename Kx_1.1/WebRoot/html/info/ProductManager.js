		...
		ctrl.Toolbar.Del.click(function() {
			...
			if(row) {
				$.messager.confirm('确认删除','将要删除此记录,删除操作不可恢复,请确认?',function(b){
					if(b) {
						$.post('../info/info-category!delete.action',{"input.id":row.type4},function(data){
							var result = eval('(' + data + ')');
							if(result.success)
								ctrl.ListTable.datagrid('reload');
							else
		...
		ctrl.Toolbar.Edit.click(function() {
			...
			if(row) {
				$.messager.prompt('修改','只允许修改类别名称,请输入新的名称',function(val){
					if(val) {
						$.post('../info/info-category!edit.action',{"input.id":row.type4,'input.cname':val},function(data){
							var result = eval('(' + data + ')');
							if(result.success)
								ctrl.ListTable.datagrid('reload');
							else
